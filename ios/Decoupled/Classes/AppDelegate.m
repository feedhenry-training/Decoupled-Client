...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    if (url && [url isKindOfClass:[NSURL class]]) {
        ...
        NSLog(@"Decoupled launchOptions = %@", url);
    }
    ...
    NSString *typeSubString = [deviceType substringWithRange:NSMakeRange(0, 4)];
    NSLog(@"Device type: %@", typeSubString);

    if([typeSubString isEqualToString:@"iPad"]){
        self.viewController.wwwFolderName = @"www/tablet";
    }else{
        self.viewController.wwwFolderName = @"www/phone";   
    }
...
// only valid if Decoupled-Info.plist specifies a protocol to handle
